...
public class BaseDefaultNetFragment extends BaseNetFragment {
    @Override
    ...
    private void updateApp(Result result) {
        ...
        if (androidType == 1) {//不强制升级
            ...
            if (showUpdate) {
                DownloadManager download = new DownloadManager(getActivity(), false);
                download.requestAndDown();
            }
        } else if (androidType == 2) {//强制升级
            DownloadManager download = new DownloadManager(getActivity(), true);
            download.requestAndDown();
        }
    }
