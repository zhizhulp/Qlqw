...
public class HorizontalStepsViewIndicator extends View {
    ...
    private int mUnCompletedLineColor = ContextCompat.getColor(getContext(), R.color.grey_line);
    private int mCompletedLineColor = ContextCompat.getColor(getContext(), R.color.blue_tv);//定义默认完成线的颜色      definition mCompletedLineColor
    ...
    private void init() {
        ...
        mLinePadding = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 90, getResources().getDisplayMetrics());
        mCompleteIcon = ContextCompat.getDrawable(getContext(), R.mipmap.cg_over);//已经完成的icon
        mAttentionIcon = ContextCompat.getDrawable(getContext(), R.drawable.attention);//正在进行的icon
        mDefaultIcon = ContextCompat.getDrawable(getContext(), R.mipmap.cg_no_over);//未完成的icon
    }
    ...
    protected synchronized void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < mCircleCenterPointPositionList.size() - 1; i++) {
            ...
            if (i <= mComplectingPosition - 1 && mStepBeanList.get(0).getState() != StepBean.STEP_UNDO)//判断在完成之前的所有点
            {
                ...
                canvas.drawRect(preComplectedXPosition + mCircleRadius - 10, mLeftY,
                        afterComplectedXPosition - mCircleRadius + 10, mRightY, mUnCompletedPaint);
