...
public class ICamera {
    ...
    public Camera openCamera(Activity activity) {
        try {
            ...
            Camera.Size bestPreviewSize = calBestPreviewSize(mCamera.getParameters(), dm.heightPixels,  dm.widthPixels);
            //Camera.Size bestPreviewSize = calBestPreviewSize(mCamera.getParameters(), 960,  540);
    ...
    public RelativeLayout.LayoutParams getLayoutParam() {
        ...
        float scale = Math.max(Screen.mWidth * 1.0f / previewSize.height,
                Screen.mHeight * 1.0f / previewSize.width);
        Log.d(TAG, "getLayoutParam_preview: "+previewSize.width+"*"+ previewSize.height);
        Log.d(TAG, "getLayoutParam_screen: "+Screen.mHeight+"*"+Screen.mWidth);
        Log.d(TAG, "getLayoutParam_scale: "+scale);
        int layout_width = (int) (scale * previewSize.height);
        int layout_height = (int) (scale * previewSize.width);
        Log.d(TAG, "getLayoutParam_final: "+layout_height+"*"+layout_width);
        RelativeLayout.LayoutParams lp = new RelativeLayout.LayoutParams(layout_width, layout_height);
        lp.addRule(RelativeLayout.CENTER_HORIZONTAL);
        return lp;
        //return new RelativeLayout.LayoutParams(previewSize.height, previewSize.width);
    }
    ...
    private Camera.Size calBestPreviewSize(Camera.Parameters camPara,
        ...
        for (Camera.Size tmpSize : allSupportedSize) {
            Log.d(TAG, "allSize:"+tmpSize.width+"*"+tmpSize.height);
